...
function sortSkillRows(a, b) {
  if (a.allowed !== b.allowed) return a.allowed ? -1 : 1;
  if ((b.invested ?? 0) !== (a.invested ?? 0)) return (b.invested ?? 0) - (a.invested ?? 0);
...
function enrichSkillRow(actor, [key, skill]) {
  ...
  const remaining = actor.getRemainingSkillPoints?.() ?? 0;
  ...
  const rowTooltipParts = [
    ...
    `Máximo: ${max}`,
    `Coste actual: ${cost}`,
    `Gastado en esta skill: ${spent}`
  ];

  if (blockReason) rowTooltipParts.push(`Bloqueo: ${blockReason}`);
  else if (allowed) rowTooltipParts.push("Competencia disponible para invertir");

  const rowTooltip = rowTooltipParts.join(" | ");

  const plusTooltip = canIncrease
    ? `Subir ${skill.label} cuesta ${cost} punto${cost === 1 ? "" : "s"}`
    : (blockReason || `No puedes subir ${skill.label}`);

  const minusTooltip = canDecrease
    ? `Reducir ${skill.label} en 1 punto invertido`
    : `No hay puntos invertidos que retirar`;

  const rollTooltip = `Tirar ${skill.label} (${skill.total}%)`;
  ...
  return {
    ...
    rowClasses: [
      `skill-row--${state}`,
      allowed ? "skill-row--allowed" : "skill-row--disallowed",
      Number(skill.invested ?? 0) > 0 ? "skill-row--invested" : ""
    ].filter(Boolean).join(" "),
