import {
  ...
  postSimpleMessage,
  postAttackFailureMessage,
  postAttackHitMessage,
  clampPercent,
  DIFFICULTY_MODS,
  applyCriticalDamageBonus,
  resolveCriticalEffect,
...
function mergeCombatDefaults(combat = {}) {
  return {
    ...
    effects: Array.isArray(combat.effects) ? combat.effects : [],
...
canSpendCombatAction(cost = 1) {
  const safeCost = Math.max(0, Number(cost ?? 1));
  const current = Number(this.system.combat?.actions?.current ?? 0);
  return current >= safeCost;
}
...
async spendCombatAction(cost = 1) {
  const safeCost = Math.max(0, Number(cost ?? 1));
  const current = Number(this.system.combat?.actions?.current ?? 0);
  const next = Math.max(0, current - safeCost);
...
async addStatusEffect(effectData = {}) {
  const current = Array.isArray(this.system.combat?.statusEffects)
    ? [...this.system.combat.statusEffects]
    : [];

  current.push({
    type: String(effectData.type ?? "").trim(),
    location: String(effectData.location ?? "").trim(),
    timestamp: Date.now()
  });

  await this.update({
    "system.combat.statusEffects": current
  });
...
hasStatusEffect(type) {
  const effects = Array.isArray(this.system.combat?.statusEffects)
    ? this.system.combat.statusEffects
    : [];
...
async resolveWeaponAttack({
  ...
  if (this.hasStatusEffect("brazo-inutilizado")) {
  ui.notifications.warn("No puedes atacar: tienes un brazo inutilizado.");
  return null;
}
  ...
  if (!targetActor?.applyDamageToLocation) {
    ui.notifications.warn("El objetivo no puede recibir daño con este sistema.");
    return null;
  }
  ...
  const attackRoll = await rollPercent(this, attackTarget, `Ataque con ${weapon.name}`);
	...
	const defenseData = targetActor.getDefenseSourceData();
        ...
        if (!defenseData.valid && defenseData.reason === "arma-no-defensiva") {
  ui.notifications.warn(`${targetActor.name} tiene seleccionada un arma no defensiva para parar.`);
} 


	if (defenseData.valid) {
  	 const extraDefenseMod = Number(defenseModifier ?? 0);

  	 const defenseTarget = clampPercent(
    	 Number(defenseData.target ?? 0) +
    	 Number(defenseData.modifier ?? 0) +
     	extraDefenseMod
  	);
  	...
  	defenseRoll = await rollPercent(
    	 targetActor,
    	 defenseTarget,
    	 `Defensa: ${defenseData.label}`
  );
}

  const comparison = compareAttackDefense(attackRoll, defenseRoll);

console.log("DEBUG comparison justo antes del if:", comparison);

await this.spendCombatAction(1);

if (!comparison.hit) {
console.log("DEBUG RAMA FALLO");
 
await postAttackFailureMessage({
  attacker: this,
  target: targetActor,
  weapon,
  comparison,
  ...
  console.log("DEBUG RAMA IMPACTO");
  ...
  hitLocation,
  { ignoreArmor: attackRoll.critical }
);

let criticalEffect = null;

if (attackRoll.critical) {
  criticalEffect = resolveCriticalEffect({
    location: hitLocation,
    damage: applied.finalDamage,
    pvAfter: applied.pvAfter
  });

  if (criticalEffect?.effect) {
    await targetActor.addStatusEffect({
      type: criticalEffect.effect,
      location: hitLocation
    });
  }

  if (criticalEffect?.effect === "muerte-instantanea") {
    await targetActor.update({
      "system.secondary.pv.value": -999
    });

    applied.state = "muerto";
    applied.pvAfter = -999;
  }
}

await postAttackHitMessage({
  attacker: this,
  target: targetActor,
  weapon,
  attackRoll,
...
async applyCombatEffects(effects = []) {
  const current = Array.isArray(this.system.combat?.effects)
  ...
  await this.update({
    "system.combat.effects": current
  });
}

async processCombatEffects() {
  let effects = Array.isArray(this.system.combat?.effects)
    ? [...this.system.combat.effects]
    : [];
  ...
  for (const effect of effects) {
    if (effect.type === "bleeding") {
      totalDamage += Number(effect.value ?? 0);
      messages.push(`🩸 Sangrado: ${effect.value}`);
    }

    if (effect.type === "stunned") {
      messages.push(`🤕 Aturdido`);
    }

    effect.duration = Number(effect.duration ?? 1) - 1;
  }
  ...
  // aplicar daño si hay
  if (totalDamage > 0) {
    const applied = await this.applyDamageToLocation(totalDamage, "torso", { ignoreArmor: true });

    messages.push(`💀 Daño por efectos: ${applied.finalDamage}`);
  }

  await this.update({
    "system.combat.effects": effects
  });
  ...
  if (messages.length) {
    await ChatMessage.create({
      ...
      content: `
        <div class="aquelarre-chat effects">
...
async applyDamageToLocation(rawDamage, location, { ignoreArmor = false } = {}) {
  ...
  const res = Number(this.system.characteristics?.res?.value ?? 10);
  const newPv = Math.max(-res, currentPv - finalDamage);
  await this.update({
...
getDefenseSourceData() {
  ...
  const hasBrokenLeg = this.hasStatusEffect("pierna-inutilizada");
  const legPenalty = hasBrokenLeg ? -20 : 0;
  ...
  if (mode === "weapon") {
    ...
    return {
      label: `${weapon.name}`,
      target: clampPercent(data.target + parryBonus + legPenalty),
      modifier: defenseModifier,
      valid: !!data.skill
    };
  }
  ...
  if (mode === "shield") {
    ...
    return {
      label: `${shield.name}`,
      target: clampPercent(data.target + bonus + legPenalty),
      modifier: defenseModifier,
      valid: !!data.skill
    };
  }

  const data = this.getSkillTarget(
    normalizeSkillKey(this.system.combat?.defenseSkill, "esquivar")
  );

  return {
    label: data.skill?.label ?? "Defensa",
    target: clampPercent(data.target + legPenalty),
    modifier: defenseModifier,
    valid: !!data.skill
  };
}
