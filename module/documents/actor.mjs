...
const hitLocation = getHitLocation(locationRoll.total);
  ...
  if (defenderSkill) {
    ...
    const defenseTarget = clampPercent(Number(defenderSkill.total ?? 0) + defMod);
    defenseRoll = await rollPercent(targetActor, defenseTarget, `Defensa: ${defenderSkill.label}`);
  }
...
getDifficultyMod() {
  ...
  async rollWeaponDamage(itemId) {
    ...
    const damage = await rollFormula(this, weapon.system.damage || "1d3", `Daño: ${weapon.name}`);

    await postSimpleMessage(this, `Daño: ${weapon.name}`, [
      `<strong>Fórmula:</strong> ${damage.formula}`,
      `<strong>Resultado:</strong> ${damage.total}`
    ]);
  ...
  async consumeAction() {
    const current = Number(this.system.combat?.actions?.current ?? 0);
    await this.update({ "system.combat.actions.current": Math.max(0, current - 1) });
  }

  async resetActions() {
    await this.update({ "system.combat.actions.current": 2 });
  }

  getArmorForLocation(location) {
    const equippedArmor = this.items.filter(i => i.type === "armor" && i.system.equipped);
    const equippedShields = this.items.filter(i => i.type === "shield" && i.system.equipped);

    const armor = equippedArmor
      .filter(item => {
        const loc = String(item.system.location || "").toLowerCase();
        return loc === "general" || loc === String(location).toLowerCase();
      })
      .reduce((sum, item) => sum + Number(item.system.protection || 0), 0);

    const shieldProt = equippedShields.reduce((sum, item) => sum + Number(item.system.protection || 0), 0);
    return armor + shieldProt;
  }

  async applyDamage(amount) {
    const currentPV = Number(this.system.secondary?.pv?.value ?? 0);
    const newPV = Math.max(0, currentPV - Math.max(0, Number(amount || 0)));
    await this.update({ "system.secondary.pv.value": newPV });
    return newPV;
  }

  async resolveAttackAgainst(targetActor, weaponId) {
    if (!targetActor || !(targetActor instanceof Actor)) return;

    await this.ensureActorDefaults();
    await targetActor.ensureActorDefaults?.();

    const weapon = this.items.get(weaponId);
    if (!weapon || weapon.type !== "weapon") return;

    const myActions = Number(this.system.combat?.actions?.current);
    if (!Number.isFinite(myActions) || myActions <= 0) {
      ui.notifications?.warn(`${this.name} no tiene acciones disponibles.`);
      return;
    }

    const attackData = this.getSkillTarget(normalizeSkillKey(weapon.system.skill, "espadas"));
    if (!attackData.skill) {
      ui.notifications?.warn(`El arma ${weapon.name} usa una competencia no válida: ${weapon.system.skill}`);
      return;
    }

    const attackModifier = Number(this.system.combat?.attackModifier ?? 0) + this.getDifficultyMod();
    const finalAttackTarget = clampPercent(attackData.target + attackModifier);
    const attackRoll = await rollPercent(this, finalAttackTarget, `Ataque: ${weapon.name}`);
    ...
    const targetActions = Number(targetActor.system.combat?.actions?.current ?? 0);
    if (targetActions > 0) {
      const defense = targetActor.getDefenseSourceData();

      if (defense.valid) {
        defenseBase = defense.target;
        defenseModifier = defense.modifier + targetActor.getDifficultyMod();
        defenseLabel = defense.label;

        const finalDefenseTarget = clampPercent(defense.target + defenseModifier);
        defenseRoll = await rollPercent(targetActor, finalDefenseTarget, `Defensa: ${defense.label}`);
        await targetActor.consumeAction?.();
      } else if (defense.reason === "arma-no-defensiva") {
        ui.notifications?.warn(`${targetActor.name} tiene seleccionada un arma no defensiva para parar.`);
      }
    }
    ...
    if (attackRoll.critical) {
      criticalEffect = getCriticalEffect();
      await this.update({ "system.combat.lastCritical": criticalEffect });
    }

    if (attackRoll.fumble) {
      fumbleEffect = getFumbleEffect();
      await this.update({ "system.combat.lastFumble": fumbleEffect });
    }

    if (defenseRoll?.fumble && !fumbleEffect) {
      fumbleEffect = getFumbleEffect();
      await targetActor.update({ "system.combat.lastFumble": fumbleEffect });
    }

    if (!comparison.hit) {
      await postCombatMessage({
        attacker: this,
    ...
    const locRoll = await rollFormula(this, "1d100", "Localización");
    const location = getHitLocation(locRoll.total);

    const damageRoll = await rollFormula(this, weapon.system.damage || "1d3", `Daño: ${weapon.name}`);
    const criticalBonus = attackRoll.critical ? applyCriticalDamageBonus(damageRoll.total) : 0;
    const grossDamage = Number(damageRoll.total) + criticalBonus;
    const armor = targetActor.getArmorForLocation?.(location) ?? 0;
    const netDamage = Math.max(0, grossDamage - Number(armor));
    const remainingPV = await targetActor.applyDamage(netDamage);
